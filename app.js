...
app.get('/jokes/filter' , (req , res)=>{
    ...
    console.log(query)
    if (query){
      const filteredJokes = jokes.filter((joke) => joke.jokeType.toLowerCase() === query.toLowerCase());
      res.json(filteredJokes);
    }else{
      res.status(404).json({ message: "No jokes found with that type" });
    }
})
app.get('/jokes/:id' , (req , res) =>{
    ...
    if(joke){
      ...
      res.status(404).json({ message: `Joke with id:${req.params.id} not found. Try creating a new one!` });
    }
})
...
app.put('/jokes/edit/:id' , (req , res) =>{
  ...
  if(joke){
    ...
    joke.jokeType = newType;
    joke.jokeText = newText;
    res.json(joke);
  }else{
    res.status(404).json({ message: `Joke with id:${req.params.id} not found. Try creating a new one!` });
  }
})
app.patch('/jokes/edit/:id' , (req , res) =>{
  const joke = jokes.find((joke) => joke.id == req.params.id)
  if(joke){
    const newType = req.body.type
    const newText = req.body.text
    joke.jokeType = newType ? newType : joke.jokeType;
    joke.jokeText = newText ? newText : joke.jokeText;
  }else{
    res.status(404).json({ message: `Joke with id:${req.params.id} not found. Try creating a new one!` });
  }
    res.json(joke);
